...
class SQLAlchemyComponent(Component):
    ...
    * one :class:`~sqlalchemy.orm.session.Session` resource factory
    ...
    def configure_engine(
        ...
        bind: Union[Connection, Engine] = None,
    ...
    async def start(self, ctx: Context):
        if isinstance(self.commit_executor, str):
            self.commit_executor = await ctx.request_resource(
                Executor, self.commit_executor
            )
        ...
        for (
            ...
            ctx.add_resource_factory(
                partial(self.create_session, factory=factory),
                [Session],
                resource_name,
                context_attr,
            )
            logger.info(
                "Configured SQLAlchemy session maker (%s / ctx.%s; dialect=%s)",
                resource_name,
                context_attr,
                bind.dialect.name,
            )
        ...
        for (
            ...
            logger.info(
                "SQLAlchemy session maker (%s / ctx.%s) shut down",
                resource_name,
                context_attr,
            )
